...
namespace Flee.CalcEngine.InternalTypes
{
    ...
    internal class DependencyManager<T> where T : notnull
    {
        ...
        private Dictionary<T, object?> CreateInnerDictionary()
        {
            return new Dictionary<T, object?>(_myEqualityComparer);
        }

        private Dictionary<T, object?>? GetInnerDictionary(T tail)
        {
            if (_myDependentsMap.TryGetValue(tail, out var value))
            {
                return value;
            }
            else
        ...
        {
            Dictionary<T, object?> seenNodes = this.CreateInnerDictionary();
        ...
        private void CloneDependentsInternal(T tail, DependencyManager<T> target, Dictionary<T, object?> seenNodes)
        {
            ...
            Dictionary<T, object?>? innerDict = this.GetInnerDictionary(tail);
        ...
        public void AddTail(T tail)
        {
            ...
            {
                _myDependentsMap.Add(tail, this.CreateInnerDictionary());
            }
        }
        ...
        public void AddDepedency(T tail, T head)
        {
            ...
            if (innerDict != null && innerDict.ContainsKey(head) == false)
            {
        ...
        private void RemoveHead(T head, Dictionary<T, object?> dict)
        {
        ...
        {
            Dictionary<T, object?> dependents = this.CreateInnerDictionary();
        ...
        private void GetDependentsRecursive(T tail, Dictionary<T, object?> dependents)
        {
            ...
            Dictionary<T, object?>? directDependents = this.GetInnerDictionary(tail);
        ...
        public void GetDirectPrecedents(T head, IList<T> dest)
        {
            ...
            {
                Dictionary<T, object?>? innerDict = this.GetInnerDictionary(tail);
                if (innerDict != null && innerDict.ContainsKey(head) == true)
                {
                    dest.Add(tail);
                }
            }
        ...
        {
            Dictionary<T, object?>? innerDict = this.GetInnerDictionary(tail);
            return innerDict != null && innerDict.Count > 0;
        }
